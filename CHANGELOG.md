...
### Fixed
- **Custom Site Category Assignment & Reversion Fix**: Fixed a bug where changing a site's category dropdown (e.g., re-categorizing YouTube as Learning) snapped back to its default category after a few milliseconds. Removed legacy block-rule categorization overrides from the background service worker, eliminated an un-awaited async storage race condition, normalized apex (`youtube.com`) and subdomain (`www.youtube.com`) storage keys, and exposed module-scoped `siteCategories` variables to `window.siteCategories` so global resolution functions (`getEffectiveCat`) read custom choices correctly.
- **Domain List Question Mark (`?`) Indicator Removal**: Removed the auto-categorized question mark (`?`) badge next to domain names across Top Sites and Site Manager tables for a cleaner visual layout.
