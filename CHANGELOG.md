...
### Added
- **Universal Modern Tab UI Redesign (Shadcn / Base-UI Architecture)**: Replaced legacy tab styling across the entire extension with a modern, unified tab system inspired by Shadcn and Base-UI design systems. Features borderless floating card pills (`var(--bg2)` in dark mode, `#ffffff` in light mode), soft hover states (`var(--bg4)`), 200ms cubic-bezier transition physics, zero green glow/shadow overlays, and zero focus ring outlines on tab clicks across Popup Header (`Today` / `All Time`), Dashboard Header Nav (`Analytics`, `Focus Mode`, `Site Manager`, `Settings`), Analytics Sub-Tabs (`Overview`, `Daily Breakdown`, `Top Sites`, `Study vs Distraction`), Site Manager Sub-Tabs (`Site List`, `Smart Presets & Categories`, `Advanced Tweaks`), Rule Manager Sub-Tabs (`Block List`, `Allowlist`, `Never Track`), Settings Section Tabs (`Security & System`, `Tracking & Blocking`), and Date Range Selector pills (`7d`, `14d`, `30d`, `45d`).
...
### Fixed
- **Popup & Dashboard Layout Overlapping & Spacing Fixes**: Resolved tab bar and search container collisions across the dashboard. Added a clean 20px bottom margin (`margin-bottom: 20px`) under `.view-tabs` and `.sm-tabs` to separate main sub-tabs from date range pills, added a 16px left/right inset (`margin: 16px 16px 0; width: calc(100% - 32px)`) on popup time tabs (`.time-tabs`) for clean alignment with card containers below, and separated Rule Manager tab tracks from search inputs.
- **Report Issue Button Polish & Dimension Alignment**: Removed bug/lightbulb SVG icons from all 4 `Report Issue` buttons for clean text presentation, and matched their height (38px), corner radius (10px), font size (13px), font weight (600), and padding (`padding: 0 16px`) to sit in 100% visual symmetry next to `.ff-search` input cards.
...
- **Extension Popup & Dashboard Startup Shake Fix**: Eliminated popup frame shaking on open by adding `min-height: 520px` to the popup body, and prevented simulated navigation button clicks on dashboard page load that were stripping active classes and restarting animations mid-frame.
- **Analytics Sub-Tab First-Click Blink/Vibrate Fix**: Resolved a first-click animation jank issue where `loadAnalytics()` data fetching ran synchronously before the animation class was applied, causing a 300ms blank screen followed by a late blink. Refactored sub-tab click listeners to trigger `.ff-tab-anim` animation immediately on Frame 0 (0ms) upon click, with data loading running asynchronously in the background without blocking the animation.
- **Analytics Sub-Tab Vertical Layout Jump Fix**: Standardized the control bar header height across all 4 analytics sub-tabs (Overview, Daily Breakdown, Top Sites, Comparison) with a unified `.atab-header-row` class (`min-height: 38px`, `margin-bottom: 20px`), eliminating an 18px vertical content jump caused by Top Sites having a taller search input and Report Issue button row.
- **Loading State Height Expansion Jump Fix**: Added `min-height: 420px` to `.ff-loading` spinner containers and `min-height: 450px` to all analytics data containers (`#top-sites`, `#daily-breakdown-list`, `#atab-trend`, `#atab-overview`) to pre-reserve vertical space during data loading, preventing layout expansion jumps when table rows and charts render.
- **CSS Containment & GPU Layer Isolation for Animations**: Added `contain: content` and `backface-visibility: hidden` to `.tab.act`, `.ff-tab-anim`, and `.sm-pane.is-active` animated containers, isolating the animation compositing layer from DOM mutations (chart rendering, table injection) happening inside the container during first-click transitions.

### Changed
- **Smooth Tab Transition Animations**: Added silky slide-and-fade animations across all tab navigation. Main tab sections (Analytics, Focus Mode, Site Manager, Settings) animate with a 0.9s vertical spring glide (`ffMainTabSlide`), and sub-tabs (Overview, Daily Breakdown, Top Sites, Comparison, Site Manager panes) animate with a 0.8s vertical spring glide (`ffSubTabSlide`). Both use `cubic-bezier(0.16, 1, 0.3, 1)` deceleration easing for a relaxed, premium feel.
